...
public class GameObjectPoolManager : MonoBehaviour
{
    ...
    public GameObject GetGameObjectDirect(string path)
    {
		...
		if (resourcePools.ContainsKey (path)) {
			...
			Object asset = AssetLoaderManager.Instance.LoadResourceBlock(path);
			if (asset != null)
			{
				GameObjectPoolManager.Instance.Regist(path,1,asset);
				obj = GameObjectPoolManager.Instance.Spawn(path);
			}
		}
    ...
    /// </summary>
	public void GetGameObjectAsync(string path, Void_GameObject finishCallback)
    {
		if (resourcePools.ContainsKey (path)) {
			var obj = GameObjectPoolManager.Instance.Spawn(path);
			finishCallback (obj);
		} else {
			AssetLoaderManager.Instance.LoadResourceAsync(path, (string resPath,UnityEngine.Object asset) => 
				{
					GameObject obj = null;
					if (asset != null)
					{
	...
	public GameObjectPool Regist(string resourcePath, int preNum,Object obj)
    {
        if (resourcePools.ContainsKey(resourcePath))
        {
            resourcePools[resourcePath].count++;
			return CreateGameObjectPool( resourcePools[resourcePath].prefab,preNum);
        }
        else
        {
            resourcePools.Add(resourcePath, new AssetCounter(obj as GameObject, preNum));
			return CreateGameObjectPool(obj as GameObject, preNum);
        }
    }
    /// <summary>
    /// 使用资源路径生产一个物体
	...
	private GameObjectPool CreateGameObjectPool(GameObject obj, int num)
    {
        if (null == obj)
        {
    ...
    public void Unspawn(ref GameObject obj)
    {
        if (obj == null)
        {
            return;
    ...
    public void DestoryGameObjectPool(GameObject gObj)
    {
        ...
        {
			gameObjectPools[id].UnspawnAllAndDestroy();
