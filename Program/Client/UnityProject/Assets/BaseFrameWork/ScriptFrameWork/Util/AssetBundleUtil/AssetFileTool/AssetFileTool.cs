...
//	public const string dynamicMainFest = "dynamicmainfest";
    ...
    public static AssetFileTool LoadFromString(string data)
    {
        ...
        try
        {
            ...
            {
				if(!int.TryParse(sr.ReadLine(),out aft.version)){
					Debug.LogError("Asset file load error " + data);
					return null;
				}
                ...
                while (line != null)
                {
                    ...
                    {
                        XZXDDebug.LogWarning("Asset file error " + line);
                    }
                    line = sr.ReadLine();
                }
                sr.Close();
    ...
    /// <returns></returns>
    public static int GetResVersionFromFile(string path)
    {
        if (!File.Exists(path))
        {
    ...
    public bool SaveToFile(string path)
    {
        ...
        /// 单纯写资源
        /// </summary>
        /// <returns><c>true</c>, if assets to file was saved, <c>false</c> otherwise.</returns>
        /// <param name="path">Path.</param>
        public bool SaveAssetHashCodeToFile(string path)
        {
            try
            {
                if (File.Exists(path))
	...
	public void ChangedAsset(AssetCell ac){
		assetCells[ac.path] = ac;
	}

	public AssetFileTool ChangedAssets(List<AssetCell> acs){
	...
	public static List<AssetCell> GetAssetCellsFromString(string data){
		List<AssetCell> assetCells = new List<AssetCell>();
		try
		{
			using (StringReader sr = new StringReader(data))
			{
